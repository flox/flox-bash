{
  ...
  outputs = { capacitor, target, ... } @ args: capacitor args (context: {

    apps.analysis = {};

    config.plugins = [
      # mount the analysis attributes
      (import ./plugins/eval.nix context { inherit target; attributePath = [ "analysis" "eval" ]; })
      (import ./plugins/eval.nix context { inherit target; attributePath = [ "analysis" "build" ]; build = { analyzeOutput = true; }; })
