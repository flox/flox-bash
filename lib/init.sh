...
# Default profile "owner" directory, i.e. ~/.local/share/flox/profiles/local/default/bin
profileOwner="local" # as in "/usr/local"
if [ -L "$FLOX_PROFILES/$profileOwner" ]; then
	profileOwner=$(readlink "$FLOX_PROFILES/$profileOwner")
fi
...
catalogAttrPathPrefix="legacyPackages.$NIX_CONFIG_system"
